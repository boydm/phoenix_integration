defmodule PhoenixIntegration.LinksTest do
  ...
  test "find :get anchor in html via uri or path", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, @href_first_get, :get) ==
             {:ok, @href_first_get}

    assert PhoenixIntegration.Requests.test_find_html_link(html, @href_first_get, :get) ==
             {:ok, @href_first_get}

    assert PhoenixIntegration.Requests.test_find_html_link(html, @href_second_get, :get) ==
             {:ok, @href_second_get}
  end

  test "find :get anchor in html via #id", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, "#second", :get) ==
             {:ok, @href_second_get}
  end

  test "find :get anchor in html via text", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, "First Link", :get) ==
             {:ok, @href_first_get}

    assert PhoenixIntegration.Requests.test_find_html_link(html, "Second Link", :get) ==
             {:ok, @href_second_get}
  end
  ...
  test "find :post anchor in html via uri or path", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, @href_post, :post) ==
             {:ok, @href_post}
  end

  test "find :post anchor in html via #id", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, "#post_id", :post) ==
             {:ok, @href_post}
  end

  test "find :post anchor in html via text", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, "POST link text", :post) ==
             {:ok, @href_post}
  end
  ...
  test "find :put anchor in html via uri or path", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, @href_put, :put) ==
             {:ok, @href_put}
  end

  test "find :put anchor in html via #id", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, "#put_id", :put) ==
             {:ok, @href_put}
  end

  test "find :put anchor in html via text", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, "PUT link text", :put) ==
             {:ok, @href_put}
  end
  ...
  test "find :patch anchor in html via uri or path", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, @href_patch, :patch) ==
             {:ok, @href_patch}
  end

  test "find :patch anchor in html via #id", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, "#patch_id", :patch) ==
             {:ok, @href_patch}
  end

  test "find :patch anchor in html via text", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, "PATCH link text", :patch) ==
             {:ok, @href_patch}
  end
  ...
  test "find :delete anchor in html via uri or path", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, @href_delete, :delete) ==
             {:ok, @href_delete}
  end

  test "find :delete anchor in html via #id", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, "#delete_id", :delete) ==
             {:ok, @href_delete}
  end

  test "find :delete anchor in html via text", %{html: html} do
    assert PhoenixIntegration.Requests.test_find_html_link(html, "DELETE link text", :delete) ==
             {:ok, @href_delete}
  end
